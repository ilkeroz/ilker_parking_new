view: com_report_dwelltime_by_group_weekly {
 derived_table: {
  sql: select
...
measure: DwellTime {
  ...
  value_format: "0.00"
  sql: CASE WHEN {% condition Statistics %} 'Average' {% endcondition %} THEN ${com_report_dwelltime_by_group_weekly.Avg_Group_Dwelltime}
  ...
  link: {
    ...
    link: {
      ...
      url: "/dashboards/116?Site={{ siteName_hidden._value | url_encode}}&Group={{ parkingGroupId_hidden._value | url_encode}}&Time={{ startTime_week._value | url_encode }}+for+7+days&Statistics={{_filters['com_report_dwelltime_by_group_weekly.Statistics']}}"
    }
  }
